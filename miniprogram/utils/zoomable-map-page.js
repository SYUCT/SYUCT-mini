// Shared page factory for high-resolution map subpackages.
...
function createZoomableMapPage(maps, defaultId) {
  const mapIndex = maps || {};
  const firstId = Object.keys(mapIndex)[0];
  const defaultMap = mapIndex[defaultId] || mapIndex[firstId] || {};
  ...
  return Object.assign({
    ...
    onLoad(options) {
      ...
      const map = mapIndex[(options && options.id) || ''] || defaultMap;
      this.setData({ map });
      if (map.title) wx.setNavigationBarTitle({ title: map.title });
    },
    ...
    onScale(e) {
      const scale = Number((e && e.detail && e.detail.scale) || MIN_SCALE);
...
module.exports = { createZoomableMapPage };
